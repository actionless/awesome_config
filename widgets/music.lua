...
-------------------------------------------------------------------------------
	...
	player.widget:connect_signal("mouse::enter", function () player.show_notification() end)
	player.widget:connect_signal("mouse::leave", function () player.hide_notification() end)
	player.widget:buttons(awful.util.table.join(
	...
	function player.clementine_update(f)
		...
		for line in f:lines() do
			...
			elseif k == "arturl" then
				player_status.cover  = string.match(v, "^file://(.*)$")
			end
...
-------------------------------------------------------------------------------
	function player.post_update()
		...
		if player_status.artist == "N/A" or player_status.artist == '' then
			player_status.artist = escape_f(player_status.file:match("^(.*)['/]%d+ [-] .*")) or
			escape_f(player_status.file:match("^.*['/](.*) [-] .*")) or
			escape_f(player_status.file:match("^(.*)['/].*")) or "N/A"
		end

		if player_status.title == "N/A" or player_status.title == '' then
			player_status.title = escape_f(player_status.file:match(".*['/'].* [-] (.*)[.].*")) or
			escape_f(player_status.file:match(".*['/'](.*) [.].*")) or escape_f(player_status.file)
		end
